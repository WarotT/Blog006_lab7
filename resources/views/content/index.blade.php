...
@section('content')
    ...
    <div class="mb-2">
        ...
        <a href="{{ url('vote') }}" role="button" class="btn btn-sm btn-dark">Show All vote</a>
    </div>
    <table class="table table-bordered" id="tbContent">
        <thead>
            <tr>
                ...
                <th>Create Date</th>
                <th>Status</th>
                <th style="width: 275px" class="text-center">Actions</th>
            </tr>
        </thead>
        ...
        <tbody>
            @foreach ($contents as $content)
                <tr>
                    ...
                    <td><a href="{{ url('#') }}" class="detail-item" data-detail="{{ $content->description }}" data-topic="{{ $content->topic }}">{{ $content->topic }}</a></td>
                    <td>{{ $content->tags }}</td>
                    ...
                    <td>
                        ...
                        @if ($content->status)
                            <button type="button" class="btn btn-danger delete-item" data-id="{{ $content->id }}">Disable</button>
                        @else
                            <button type="button" class="btn btn-success delete-item" data-id="{{ $content->id }}">Enable</button>
                        @endif
                        <a href="{{ url("vote/{$content->id}") }}" role="button" class="btn btn-sm btn-success">Voting</a>
                    </td>
                </tr>
            @endforeach
        </tbody>
